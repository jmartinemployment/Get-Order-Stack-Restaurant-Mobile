...
const AUTH_TOKEN_KEY = '@orderstack_kds_auth_token';
const AUTH_USER_KEY = '@orderstack_kds_auth_user';
...
class AuthService {
  ...
  async init(): Promise<{ token: string | null; user: AuthUser | null }> {
    try {
      ...
      console.error('[KDS Auth] Failed to load stored credentials:', error);
  ...
  async login(email: string, password: string): Promise<LoginResult> {
    try {
      ...
      console.error('[KDS Auth] Login error:', error);
  ...
  async getCurrentUser(): Promise<{ user: AuthUser; restaurants: AuthRestaurant[] } | null> {
    ...
    try {
      ...
      console.error('[KDS Auth] Get current user error:', error);
      return null;
    }
  }
  ...
  async logout(): Promise<void> {
    if (this.token) {
      try {
        ...
        console.error('[KDS Auth] Logout error:', error);
      }
    }
